...
describe('SidebarChatListItem', () => {
    ...
    test("list item should display the friend's name", () => {
        const { getByText } = render(<SidebarChatListItem friend={friendFromName("Elmer")} unseenMessages={1} chatId='stub' />);
        const text = getByText("Elmer");
        expect(text).toBeInTheDocument();
    });

    test("list item should display the friend's name, different data", () => {
        const { getByText } = render(<SidebarChatListItem friend={friendFromName("Lucy")} unseenMessages={1} chatId='stub' />);
        const text = getByText("Lucy");
        expect(text).toBeInTheDocument();
    });

    test('should be a list item', () => {
        const { getByRole } = render(<SidebarChatListItem friend={friendFromName("Lucy")} unseenMessages={1} chatId='stub' />);
    ...
    test('do not display count if unseenMessages is  0', () => {
        const { queryByText } = render(<SidebarChatListItem friend={friendFromName("Lucy")} unseenMessages={0} chatId='stub' />);
        const count = queryByText('0');
        expect(count).not.toBeInTheDocument();
    })

    test('display count if unseenMessages is over 0, different amount', () => {
        const { getByText } = render(<SidebarChatListItem friend={friendFromName("Lucy")} unseenMessages={42} chatId='stub' />);
    ...
    test('link should have address of "/dashboard/chat/{output of hrefConstructor}"', () => {
        const { getByRole } = render(<SidebarChatListItem friend={friendFromName("Lucy")} unseenMessages={1} chatId={'1701--1812'} />);
        const link = getByRole('link');
    ...
    test('link should have address of "/dashboard/chat/{output of hrefConstructor}"', () => {
        const { getByRole } = render(<SidebarChatListItem friend={friendFromName("Dick")} unseenMessages={1} chatId={'batman--robin'} />);
        const link = getByRole('link');

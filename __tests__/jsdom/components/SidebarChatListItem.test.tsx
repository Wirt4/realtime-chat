...
describe('SidebarChatListItem', () => {
    ...
    test('do not display count if unseenMessages is  0', () => {
        const { queryByText } = render(<SidebarChatListItem unseenMessages={0} participants={[friendFromName("stub"), friendFromName("stub")]} chatId='stub' />);
        const count = queryByText('0');
        expect(count).not.toBeInTheDocument();
    })
    /*
        test('display count if unseenMessages is over 0, different amount', () => {
            const { getByText } = render(<SidebarChatListItem unseenMessages={42} participants={[friendFromName("stub"), friendFromName("stub")]} chatId='stub' />);
            const count = getByText('42');
            expect(count).toBeInTheDocument();
        })
    */
    test('should have a link', () => {
        const { getByRole } = render(<SidebarChatListItem unseenMessages={4} participants={[friendFromName("stub"), friendFromName("stub")]} chatId='stub' />);
        const link = getByRole('link');
        expect(link).toBeInTheDocument();
    });

    test('link should have address of "/dashboard/chat/{output of hrefConstructor}"', () => {
        const { getByRole } = render(<SidebarChatListItem unseenMessages={42} participants={[friendFromName("stub"), friendFromName("stub")]} chatId={'1701--1812'} />);
        const link = getByRole('link');
    ...
    test('link should have address of "/dashboard/chat/{output of hrefConstructor}"', () => {
        const { getByRole } = render(<SidebarChatListItem unseenMessages={0} participants={[friendFromName("stub"), friendFromName("stub")]} chatId={'batman--robin'} />);
        const link = getByRole('link');

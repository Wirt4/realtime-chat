...
describe('Accept Tests',()=>{
    ...
    it('if handleFriendRequest throws an Already friends error, return a 400', async()=>{
        mockService.handleFriendRequest = jest.fn().mockRejectedValue(FriendRequestStatus.AlreadyFriends)
        controller = new AcceptFriendsController(mockService as FriendsService)
        result = await controller.acceptFriendRequest(request);
        expect(result.status).toEqual(400)
    })
    it('if handleFriendRequest throws a No Existing Friend Request error, return a 400', async()=>{
        mockService.handleFriendRequest = jest.fn().mockRejectedValue(FriendRequestStatus.NoExistingFriendRequest)
        controller = new AcceptFriendsController(mockService as FriendsService)
        result = await controller.acceptFriendRequest(request);
        expect(result.status).toEqual(400)
    })
    it('if handleFriendRequest throws an unknown error, return a 500', async()=>{
        mockService.handleFriendRequest = jest.fn().mockRejectedValue({message:'random error'})
        controller = new AcceptFriendsController(mockService as FriendsService)
        result = await controller.acceptFriendRequest(request);
